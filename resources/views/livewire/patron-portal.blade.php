<div class="p-6 max-w-7xl mx-auto space-y-6 bg-slate-50 min-h-screen">
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-2xl bg-blue-600 text-white font-bold flex items-center justify-center text-lg shadow-md shadow-blue-500/20">
                {{ strtoupper(substr($patron->first_name ?? 'P', 0, 1)) }}
            </div>
            <div>
                <h1 class="text-xl font-bold text-slate-900">{{ $patron->first_name }} {{ $patron->last_name }}</h1>
                <p class="text-xs text-slate-500 font-medium">
                    Patron ID: <span class="text-slate-800 font-semibold">{{ $patron->patron_id }}</span> •
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="inline-flex p-1 bg-slate-100 rounded-xl border border-slate-200/60 shrink-0">
                <button
                    ...
                    class="px-4 py-2 text-xs font-semibold rounded-lg transition-all {{ $activeTab === 'opac' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">
                    🔍 OPAC (Book Catalog)
                </button>
                <button
                    wire:click="$set('activeTab', 'transactions')"
                    class="px-4 py-2 text-xs font-semibold rounded-lg transition-all {{ $activeTab === 'transactions' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">
                    📖 My Transactions
                </button>
            </div>
            ...
            <button
                wire:click="logout"
                class="px-3 py-2 text-xs font-semibold text-rose-600 hover:bg-rose-50 rounded-xl transition border border-rose-200">
                Logout
            </button>
        </div>
    </div>
    ...
    @if ($activeTab === 'opac')
        <div class="space-y-6">
            <!-- OPAC Search Bar -->
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    <input
                        type="text"
                        wire:model.live.debounce.300ms="opacSearch"
                        placeholder="Search books by title, author, or ISBN..."
                        class="w-full px-4 py-2.5 text-xs rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 text-slate-800"
                    >
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                @forelse ($catalogItems as $item)
                    @php
                        $availableCount = $item->accessions->where('status', 'Available')->count();
                        $totalCount = $item->accessions->count();
                    @endphp
                    <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between hover:border-blue-300 transition group">
                        ...
                        <div class="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
                            <span>ISBN: <strong class="text-slate-700">{{ $item->isbn ?? 'N/A' }}</strong></span>
    ...
    @if ($activeTab === 'transactions')
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-4">
            <div class="flex items-center justify-between border-b pb-4 border-slate-100">
                ...
                <select wire:model.live="transactionFilter" class="px-3.5 py-2 rounded-xl border border-slate-200 text-xs font-semibold text-slate-700 focus:border-blue-500">
