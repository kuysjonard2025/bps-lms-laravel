...
<script>
window.accessionBarChart = function(initialData) {
    return {
        chart: null,
        init() {
            this.$nextTick(() => {
                ...
                const options = {
                    ...
                    legend: { show: false },
                    series: [{ name: 'Total Items', data: initialData?.series || [] }],
                    xaxis: {
                        categories: initialData?.categories || [],
                        labels: { style: { colors: '#6B7280', fontSize: '11px' } }
                    },
                    yaxis: { labels: { style: { colors: '#374151', fontSize: '11px', fontWeight: 600 } } },
                    grid: { borderColor: '#F3F4F6', strokeDashArray: 4 },
                    tooltip: { theme: 'light', y: { formatter: (val) => (val || 0).toLocaleString() + ' Items' } }
                };
