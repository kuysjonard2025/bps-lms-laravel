<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        >
            ...
            <span>Add General Reference</span>
        </button>
    </div>
    ...
    <div class="overflow-x-auto border border-gray-200 rounded-lg shadow-xs bg-white">
        <table class="min-w-full divide-y divide-gray-200 text-left text-xs sm:text-sm text-gray-700">
            <thead class="bg-gray-50 text-gray-500 uppercase tracking-wider text-[11px]">
                <tr>
                    <th scope="col" class="px-4 py-3 font-semibold">Reference Name</th>
                    <th scope="col" class="px-4 py-3 text-right font-semibold">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
                @forelse($references as $reference)
                    <tr wire:key="reference-row-{{ $reference->id }}" class="hover:bg-gray-50/75 transition-colors">
                        <td class="px-4 py-3 font-semibold text-gray-900 whitespace-nowrap">
                            {{ $reference->name }}
                        </td>
                        <td class="px-4 py-3 text-right whitespace-nowrap space-x-1">
                            <button
                                wire:click="openEditModal({{ $reference->id }})"
                                type="button"
                ...
                @empty
                    <tr>
                        <td colspan="2" class="px-4 py-8 text-center text-gray-500">
                            No general references found matching your criteria.
                        </td>
                    </tr>
                @endforelse
            </tbody>
            ...
            <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-md z-10 max-h-[90vh] flex flex-col my-auto overflow-hidden">
                <div class="bg-gray-50 px-5 sm:px-6 py-3.5 border-b border-gray-200 flex justify-between items-center shrink-0">
                    <h3 class="text-sm font-bold text-gray-900">
                        {{ $referenceIdBeingEdited ? 'Edit General Reference' : 'Add New General Reference' }}
                    </h3>
                ...
                <form wire:submit="saveReference" class="p-5 sm:p-6 space-y-4 overflow-y-auto">
                    <div>
                        <label for="reference-name" class="block text-xs font-medium text-gray-700">Reference Name *</label>
                        <input
                            id="reference-name"
                            type="text"
                            wire:model="name"
                    ...
                    <div class="mt-6 flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3">
                        ...
                        >
                            <span wire:loading wire:target="saveReference" class="animate-spin rounded-full h-3.5 w-3.5 border-b-2 border-white"></span>
                            <span>{{ $referenceIdBeingEdited ? 'Save Changes' : 'Create Reference' }}</span>
                        </button>
                    </div>
                </form>
            </div>
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-sm z-10 p-5 sm:p-6 text-center space-y-4">
                ...
                <div>
                    <h3 class="text-base font-bold text-gray-900">Delete General Reference</h3>
                    <p class="text-xs sm:text-sm text-gray-500 mt-1">Are you sure you want to delete this general reference? This action cannot be undone.</p>
                </div>
                ...
                <div class="flex flex-col-reverse sm:flex-row justify-center gap-2 sm:gap-3 pt-2">
                    ...
                    >
                        <span wire:loading wire:target="deleteReference" class="animate-spin rounded-full h-3.5 w-3.5 border-b-2 border-white"></span>
                        <span>Delete</span>
                    </button>
                </div>
            </div>
